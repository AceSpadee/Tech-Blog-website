...
router.get("/Blogpost/:id", async (req, res) => {
  try {
    ...
    console.log(`Fetched blog post by ID ${req.params.id}:`, BlogpostData);
...
router.get("/updateBlogpost/:id", async (req, res) => {
  try {
    ...
    //const BlogpostData = 1;
    console.log(`Fetched blog post by ID ${req.params.id} for updating:`, BlogpostData);
    const selectedBlogpost = BlogpostData.get({ plain: true });
    //res.json(selectedBlogpost);
...
router.get("/profile", withAuth, async (req, res) => {
  try {
    console.log(`Fetching profile for user with ID: ${req.session.user_id}`);
    // Find the logged in user based on the session ID
    const userData = await User.findByPk(req.session.user_id, {
      attributes: { exclude: ["password"] },
      include: [{ model: Blogpost }],
    });
    console.log("Retrieved user data from the database:", userData);
    const user = userData.get({ plain: true });
    console.log(`User first name after serialization: ${user.fname}`);
    //res.json(user);
    res.render("profile", {
      ...user,
      logged_in: true,
